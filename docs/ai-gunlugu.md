...
- **Plakaların Kalıcılığı Entegrasyonu (`routes.py`):** Şoför çıkış yaptığında (`logout`) veya vardiyayı sonlandırdığında (`vardiya_bitir`) plakalarının otomatik olarak silinmesi engellendi. Ayrıca başka bir şoför plaka değiştirdiğinde eski şoförün plakasının silinmesi önlendi. Bu sayede yeni eklenen veya değiştirilen tüm otobüs plakalarının yolcu sorgulama panelindeki "Demo / Aktif Plakalar" listesinde her zaman kalıcı ve test edilebilir olarak listelenmesi sağlandı.
