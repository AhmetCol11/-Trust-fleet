...
*   **Şoför Paneli:** Şoförlerin ses tanıma (Web Speech API) ile vardiya başlatıp bitirdiği, sürüş esnasında 10 dakikada bir sesli asistan tarafından yorgunluk kontrolüne tabi tutulduğu ve acil durumlarda tek tıkla canlı GPS konumu gönderen "Panik Butonu"na erişebildiği güvenli arayüz. Yapılan tüm sesli kontrollerde, sahteciliği önlemek adına şoförün o anki coğrafi konumu otomatik olarak arka planda kayıt altına alınır.
*   **Yolcu Geri Bildirim Paneli:** Seyahat eden yolcuların sisteme e-posta ile kayıt olup güvenli oturum açtıktan sonra otobüs plakasını sorguladığı (hızlı akademik testler için aktif/demo plakaların rozetler halinde listelendiği), şoförün sürüş kalitesini 5 yıldızlı interaktif sistemle derecelendirdiği ve şüpheli durumlarda anında merkeze "Acil İhbar" gönderebildiği panel. Şifre unutma durumları için token tabanlı e-posta şifre sıfırlama sistemi entegre edilmiştir. Sistemde kötüye kullanımı ve spam gönderimleri engellemek amacıyla yolcuların değerlendirme yapma sıklığı veritabanı kontrollü olarak 10 dakikada bir olacak şekilde sınırlandırılmıştır.
*   **Kontrol Merkezi (Merkez Yönetim Paneli):** Filodaki tüm araçların canlı konumlarını OpenStreetMap (Leaflet.js) interaktif haritası üzerinde gösteren, yolcu şikayetlerini ve şoförlerin sözel yorgunluk analiz raporlarını tek ekranda toplayan, 4 sekmeli premium glassmorphic takip ekranı. Araçların harita üzerindeki seyahatleri Canlı Rota Seyahat Simülatörü ile canlandırılırken; şoförlerin periyodik ses kayıtları ve vardiya bildirimlerinin tam olarak **hangi koordinattan (enlem/boylam)** yapıldığı, Zaman Tüneli sekmesinde kırmızı neon renkli ve hareketli konum rozetleri ile kronolojik olarak gösterilmektedir.
...
*   **`Sofor` - `VardiyaSesKaydi` (One-to-Many):** Şoförün vardiya başı ve sonu yaptığı karşılaştırmalı sesli kelime analiz kayıtları şoför kimliğiyle eşleştirilir. Sürücünün vardiya başlatma/bitirme işlemleri sırasında güvenliği sağlamak amacıyla o anki coğrafi konumu da kayıt altına alınır.
...
*   **Coğrafi Konum Damgası ile Sahtecilik Koruması:** `SesLog` (periyodik kontrol ses kayıtları) ve `VardiyaSesKaydi` (vardiya başlama/bitirme kayıtları) tablolarına eklenen `konum` (VARCHAR) alanı sayesinde, her sözel beyanın yapıldığı coğrafi konum GPS verilerinden (enlem/boylam olarak) anlık çekilerek ilişkilendirilir. Bu mimari karar, şoförlerin rota dışı konuşmalarını engeller, sahteciliğin önüne geçer ve güzergah uyumluluğunu (compliance) tam olarak denetler.
